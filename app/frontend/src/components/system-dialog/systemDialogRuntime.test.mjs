...
test('confirmAction pending 时锁定，重复确认只执行一次并在成功后关闭', async () => {
  ...
  assert.deepEqual(controller.getSnapshot(), {
    kind: 'confirm', title: '删除专题', message: '删除后无法恢复', tone: 'danger', pending: true,
...
test('action 失败后原确认原位转为 alert，确认后返回 failed', async () => {
  ...
  for (const reason of unsafeReasons) {
    const unsafeResult = controller.confirmAction(confirmOptions({
    ...
    controller.acknowledge();
    assert.equal(await unsafeResult, 'failed');
  }
});
