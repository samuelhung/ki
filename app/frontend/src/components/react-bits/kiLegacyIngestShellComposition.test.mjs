...
test('top navigation follows the current route and performs real router navigation', () => {
  ...
  for (const label of ['内容采集', '专题系列', '头脑风暴', '产业链', '工具箱', '系统中枢']) {
  ...
  assert.match(shell, /pathname\.startsWith\('\/brainstorm'\)\) return 2/);
...
test('formal ingest composes a split list orbit and reusable detail workspace', () => {
  ...
  assert.match(
    shellCss,
    /\.ki-ingest-detail-pane \.ingest-detail-meta-row\s*\{[^}]*display:\s*flex[^}]*flex-wrap:\s*nowrap[^}]*overflow-x:\s*auto[^}]*white-space:\s*nowrap/s,
  );
  assert.match(
    shellCss,
    /\.ki-ingest-detail-pane \.transcript-status-inline\s*\{[^}]*min-width:\s*0/s,
  );
  assert.match(
    shellCss,
    /\.ki-ingest-detail-pane \.transcript-status-inline \.text-red-400\s*\{[^}]*overflow:\s*hidden[^}]*text-overflow:\s*ellipsis/s,
  );
