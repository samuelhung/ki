...
import { buildStudyCreatePayload, buildStudyUploadFields, createStudyDetailCache, evictStudyItems, filterStudyItems, getStudyStats, resolveStudyDeletionSelection } from '../components/cinematic-study/studyWorkspace.mjs';
...
export default function CinematicStudy() {
  ...
  function handleMaterialEvicted(id: string) {
    setMaterials((current) => evictStudyItems(current, id));
    setMistakes((current) => evictStudyItems(current, id));
    detailCacheRef.current.delete(id);
  }

  function handleDeleted(id: string) {
    const nextSelectedId = resolveStudyDeletionSelection(materials, id);
    setSelectedId(nextSelectedId);
    navigate(nextSelectedId ? `/study/${nextSelectedId}` : '/study', { replace: true });
  }
