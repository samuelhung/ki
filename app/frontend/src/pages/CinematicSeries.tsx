...
export default function CinematicSeries() {
  ...
  function open(modeToOpen: Mode = 'choose') {
    ...
    if (modeToOpen === 'manual') loadEvents();
  }
  ...
  async function loadEvents() {
    setBusy(true);
    try { const response = await apiFetch('/api/events?limit=500'); const data = await response.json(); setEvents(Array.isArray(data) ? data : []); }
    catch { setEvents([]); }
    setBusy(false);
  }
...
function SeriesDialog(props: any) {
  const validEvents = props.events.filter((event: EventItem) => event.content_type === 'event' && event.status !== 'pending' && event.status !== 'error' && (event.overview?.trim() || event.ai_summary?.trim()) && !event.title.includes('孤儿视频恢复')).filter((event: EventItem) => !props.eventQuery.trim() || event.title.toLowerCase().includes(props.eventQuery.toLowerCase()));
