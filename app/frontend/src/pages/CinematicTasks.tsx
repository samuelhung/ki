...
import { filterTasks, getTaskStats, mergeTaskSnapshot, removeTask } from '../components/cinematic-tasks/taskWorkspace.mjs';
...
export default function CinematicTasks() {
  ...
  async function judgeSelected() { if (!selected) return; setBusy('judge'); try { const response = await apiFetch(`/api/tasks/${selected.id}/judge`, { method: 'POST' }); if (!response.ok) throw new Error('AI 分析失败'); const task = await response.json(); setTasks((current) => current.map((item) => item.id === task.id ? task : item)); } catch (reason: any) { setError(reason.message || 'AI 分析失败'); } setBusy(''); }
  ...
  const index = <><div className="ingest-topic-orbit task-topic-orbit"><button className={view === 'list' ? 'is-active is-gold' : ''} onClick={() => setView('list')}><ListTodo size={14} /><span>任务</span></button><button className={view !== 'list' ? 'is-active is-cyan' : ''} onClick={() => setView('month')}><CalendarDays size={14} /><span>日历</span></button></div><label className="task-index-search"><Search size={13} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索任务" /></label><div className="task-filter-row">{Object.entries(STATUS).map(([key, label]) => <button key={key} className={statusFilter === key ? 'is-active' : ''} onClick={() => setStatusFilter(key)}>{label}</button>)}</div><div className="ingest-index-list task-index-list">{filtered.map((task, index) => <button key={task.id} className={`ingest-index-item${selected?.id === task.id ? ' is-active' : ''}`} style={{ '--index-depth-scale': 1 - Math.min(index, 10) * .026, '--index-depth-z': `${-Math.min(index, 10) * 3}px`, '--index-depth-opacity': 1 - Math.min(index, 10) * .035 } as CSSProperties} onClick={() => setSelectedId(task.id)}><div className="index-title"><b>{task.title}</b><span><em className={`is-${task.priority}`}>{PRIORITY[task.priority as keyof typeof PRIORITY]}</em></span></div><small>{SOURCE[task.source as keyof typeof SOURCE] || '手工'} · {task.due_date || '无截止日'} · {STATUS[task.status as keyof typeof STATUS]}</small></button>)}</div><div className="task-index-selects"><select value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value)}>{Object.entries(SOURCE).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><select value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value)}>{Object.entries(PRIORITY).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div></>;

  const detail = <section className="task-detail-reader">{error && <div className="task-error">{error}</div>}{view === 'list' ? selected ? <><header><span>ACTION / {selected.id.slice(-6)}</span><h1>{selected.title}</h1><div className="task-detail-meta"><em>{SOURCE[selected.source as keyof typeof SOURCE] || '手工'}</em><time>{selected.due_date || '无截止日'}</time><b className={`is-${selected.priority}`}>{PRIORITY[selected.priority as keyof typeof PRIORITY]}优先级</b></div></header><nav className="task-detail-tabs">{Object.entries(STATUS).filter(([key]) => key !== 'all').map(([key, label]) => <button key={key} className={selected.status === key ? 'is-active' : ''} onClick={() => updateStatus(key)}>{label}</button>)}</nav><div className="task-detail-body"><section><h2>任务说明</h2><p>{selected.description || '暂未补充任务说明。'}</p></section>{selected.source_id && <section><h2>来源关联</h2><a href={sourceLink(selected)}><ExternalLink size={13} />{selected.source_label || '查看原始内容'}</a></section>}<section><h2>行动轨道</h2><div className="task-filmstrip">{FLOW.map((node, index) => { const current = statusFlowIndex(selected.status); return <div key={node} className={index === current ? 'is-current' : index < current ? 'is-done' : ''}><i>{index < current ? <Check size={11} /> : <CircleDot size={11} />}</i><span>{node}</span></div>; })}</div></section><footer><button onClick={openEdit}><Pencil size={14} />编辑任务</button><button onClick={judgeSelected} disabled={busy === 'judge'}>{busy === 'judge' ? <Loader2 size={14} className="animate-spin" /> : <BrainCircuit size={14} />}AI 分析</button></footer></div></> : <div className="task-empty">{loading ? <Loader2 className="animate-spin" /> : '暂无任务'}</div> : <div className="task-calendar"><div className="task-calendar-tabs">{(['month','week','day'] as const).map((key) => <button key={key} className={view === key ? 'is-active' : ''} onClick={() => setView(key)}>{key === 'month' ? '月' : key === 'week' ? '周' : '日'}</button>)}</div>{view === 'month' && <><MonthView tasks={tasks} year={calYear} month={calMonth} onNavigate={navigateMonth} openDetail={setSelectedId} selectedDate={selectedDate} onSelectDate={setSelectedDate} onQuickCreate={openCreate} /><DayTasksBelow tasks={tasks.filter((task) => task.due_date === selectedDate)} dateStr={selectedDate} openDetail={(id) => { setSelectedId(id); setView('list'); }} /></>}{view === 'week' && <><WeekView tasks={tasks} weekStart={calWeekStart} onNavigate={navigateWeek} openDetail={(id) => { setSelectedId(id); setView('list'); }} selectedDate={selectedDate} onSelectDate={setSelectedDate} onQuickCreate={openCreate} /><DayTasksBelow tasks={tasks.filter((task) => task.due_date === selectedDate)} dateStr={selectedDate} openDetail={(id) => { setSelectedId(id); setView('list'); }} /></>}{view === 'day' && <DayView tasks={tasks} year={calYear} month={calMonth} day={calDay} onNavigate={navigateDay} openDetail={(id) => { setSelectedId(id); setView('list'); }} />}</div>}</section>;
