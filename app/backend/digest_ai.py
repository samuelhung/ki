...
def _call_deepseek_text(
    ...
    try:
        ...
        logger.warning("DeepSeek digest HTTP error: %s", msg)
        raise RuntimeError(msg)
    except Exception as e:
        msg = str(e)[:300]
        logger.warning("DeepSeek digest error: %s", msg)
        raise RuntimeError(msg)
...
def digest_dir() -> Path:
    ...
    if configured:
        ...
        path = PROJECT_ROOT / "data" / "digests"
...
- 不要输出任何前言后语，直接从 "# 每日摘要" 第一行开始"""
    ...
    # Try AI generation; fall back to simple digest if no API key or AI fails
    markdown = None
    api_key = _deepseek_api_key()
    if api_key:
        try:
