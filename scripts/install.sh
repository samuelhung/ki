...
OLD_ENV="/Users/mrh/Documents/Projects/KnowledgeIntelligence/.env"
if [ -f "$OLD_ENV" ] && [ ! -f "$DATA_DIR/.env" ]; then
    cp "$OLD_ENV" "$DATA_DIR/.env"
    success "已复制配置文件"
fi

cat > "$LAUNCHD_PLIST" << PLASTEOF
...
OLD_DATA="/Users/mrh/Documents/Projects/KnowledgeIntelligence/data"
