...
知几是一个本地优先的知识情报中心：把 RSS 情报、视频/音频/文档摄入、专题系列、头脑风暴、综合事务、知识图谱和辅导中心整合到同一个桌面/Web 系统里。
...
- 当前后端/Web 生产部署为 `2.0.0+106`，来源提交为 `67611ef27120e507cf71751cc3f5c940337fc209`，访问地址为 `http://10.8.0.105:9120`。
- 当前受保护的回滚版本为 `2.0.0+104`，本次部署前数据库备份为 `deploy-20260803-074854.sqlite`；历史版本目录与部署数据库备份均完整保留。
...
准备依赖时允许延长到 10 分钟。入口会在本机 Node/npm 版本不匹配时临时使用
锁定的 Node 22.17.0 和 npm 10.9.2，首次下载后复用 npm 缓存。切换前失败不会
停止服务；切换后失败会恢复
